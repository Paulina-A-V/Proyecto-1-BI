...
def obtener_clasificaciones():
    try:
        ...
        df["clasificacion"] = df["Label"].apply(lambda x: "VERDADERO" if int(x) == 1 else "FALSO")
        ...
        noticias = noticias.replace([np.inf, -np.inf], np.nan).fillna("")

        # ⬇️ SOLO ENVÍA LAS PRIMERAS 100
        return noticias.head(100).to_dict(orient="records")
